...
class CaliperReaderWithModule:
    """Read in a Caliper file (`cali` or split JSON) or file-like object."""

    def __init__(self, filename_or_stream, query="", module_filename=""):
    ...
    def read_module_json(self):
        with open(self.module_filename) as module_json:
            module_obj = json.load(module_json)
        
        for idx, obj in enumerate(module_obj['mapping']):
            self.module_map[obj['label']] = obj['module']    
    ...
    def read(self):
        ...
        with self.timer.phase("read module json"):
            self.read_module_json()
        ...
        elif "module" not in self.df_json_data.columns:
            # for idx, row in self.df_json_data.iterrows():
            #     nid = int(row['nid'])
            #     name_from_nid = self.idx_to_node[nid]['name']
            #     row['module'] = self.module_map[name_from_nid] 
            self.df_json_data['module'] = self.df_json_data['nid'].apply(lambda nid: self.module_map[self.idx_to_node[nid]['name']])
        ...
        # append the module column to the dataframe.
